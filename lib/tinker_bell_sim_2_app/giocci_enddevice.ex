...
defmodule EndDevice do
    ...
    state =
      Map.update!(state, :algo, fn _ ->
        case algonum do
          ...
          # x when 0 <= x and x <= 0 -> "taskque"
          # x when 2 <= x and x <= 2 -> "delay"
          # x when 3 <= x and x <= 3 -> "bandwidth"
          x when 0 <= x and x <= 0 -> "responsetime"
          # x when 4 <= x and x <= 4 -> "clusterfee"
          # 指定回数の試行が終わったらエラーを吐かせて止めます
          _ -> raise "end"
        end
      end)
  ...
  def handle_cast(:create_task, state) do
    ...
    # タスク生成間隔時間
    :timer.sleep(5000 + timerrand)
    # create task ↓
    florand = :rand.uniform(5000)

    task = %{
      flo: 2500 + florand,
      task_produced_time: :erlang.monotonic_time(),
      algo: state.algo,
      restime_limit: 10000
    }
