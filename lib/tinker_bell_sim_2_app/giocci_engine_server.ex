...
defmodule GEServer do
  ...
  def handle_call(:initialize_engineinfo, _from, state) do
    ...
    flops =
      if state.flopsflag do
        5000 + :rand.uniform(10000)
      else
    ...
    state =
      Map.merge(state, %{
        taskque: [],
        task_assigned_time: [],
        hidden_parameter_flops: flops,
        processing_tasks_flag: False
      })
  ...
  def handle_cast({:assign_task_to_engine, task, rtr_delay}, state) do
    ...
    state =
      Map.update!(state, :taskque, fn x ->
        x ++ [{task.flo, task.task_produced_time, rtr_delay, task.fee_for_this_task}]
      end)
  ...
  def handle_cast(:process_a_task, state) do
    ...
    GenServer.cast(
      state.relaypid,
      {:send_task_response_time_in_cluster, hdtask, task_response_time_in_cluster}
    )
    ...
    state =
      Map.update!(state, :processing_tasks_flag, fn _ ->
        if length(state.taskque) == 0 do
          False
        else
          True
